...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T)) {
  ...
  for (int i=0;i<count;i++){
    if (data[i]==item){
      for (int u=i;u<count-1;i++){data[u]=data[u+1];}
      count--;
      return true;
    }
  }
...
string result="[";
  for (int i=0;i<count-1;i++){result+=item2str(data[i])+",";}
...
void XArrayList<T>::checkIndex(int index) {
   ...
   */
  // 
  if (index>count || index<0){cout<<"out_of_range exception";}
