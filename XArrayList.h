...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T)) {
  ...
  for (int i=0;i<count;++i){
    if (equals(item,data[i],this->itemEqual)){
      ...
      for (int u=i;u<count-1;u++){data[u]=data[u+1];}
      --count;
      return true;
    }
  }
...
int XArrayList<T>::indexOf(T item) {
  // TODO
  for (int i=0;i<count;i++){
    if (equals(item,data[i],this->itemEqual)){return i;}
  }
...
bool XArrayList<T>::contains(T item) {
  // TODO
  for (int i=0;i<count;i++){
    if (equals(item,data[i],this->itemEqual)){return true;}
  }
  return false;
}
...
void XArrayList<T>::checkIndex(int index) {
   ...
   */
  // 
 if (index<0||index>count){cout<<"Index is out of range!";}
