...
class DLinkedList : public IList<T> {
 ...
 public:
  ...
  class BWDIterator{
    ...
    Node *currentNode;
    public:
BWDIterator(DLinkedList<T> *pList=nullptr, bool begin=false):pList(pList) {
      if (begin) {
        if (pList != nullptr)
    ...
    BWDIterator &operator=(const BWDIterator &bwditerator) {
      this->currentNode = bwditerator.currentNode;
   ...
   void remove(void (*removeItemData)(T) = nullptr) {
      ...
      Node *pPrev = currentNode->next;  
      if (removeItemData != nullptr) removeItemData(currentNode->data);
      delete currentNode;
      currentNode = pPrev;
      pList->count-- ;
    }
    ...
    bool operator!=(const BWDIterator &bwditerator) {
      return currentNode != bwditerator.currentNode;
    }
...
typename DLinkedList<T>::Node *DLinkedList<T>::getPreviousNodeOf(int index) {
  ...
  if (index<=count/2){
    Node*cu=head;
    for (int i=0;i<index-1;i++){cu=cu->next;}
    return cu;
  } else {Node*cu=tail;
    for (int i=0;i<index;i++){cu=cu->prev;}
    return cu;}
}
...
int DLinkedList<T>::indexOf(T item) {
  ...
  for (int i=0;i<count;i++){
    ...
    if (equals(current->data,item,this->itemEqual)) {return i;}
    current=current->next;
  }
  return -1;
}
...
bool DLinkedList<T>::contains(T item) {
  ...
  for (int i=0;i<count;i++){
    current=current->next;
    if (equals(current->data,item,this->itemEqual)){return true;}
